...
#define CGC_ROM_ZERO_WAIT_CYCLES (0U)              ///< Specify zero wait states for ROM
#define CGC_ROM_ONE_WAIT_CYCLES  (1U)              ///< Specify one wait states for ROM
#define CGC_ROM_TWO_WAIT_CYCLES  (2U)              ///< Specify two wait states for ROM
...
__STATIC_INLINE void bsp_clocks_sram_wait_set (uint32_t setting)
{
    ...
    R_SRAM->RAMWTSC_b.ECCRAMWRWTEN = (uint8_t)(setting & 0x01);
...
static void bsp_clock_set_prechange(bsp_clock_set_callback_args_t * p_args)
{
    ...
    /** If the requested_frequency <= 80 MHz 0 ROM wait states are required) */
    if (requested_frequency <= CGC_SYS_CLOCK_FREQ_ONE_ROM_WAITS)
    {
        p_args->new_rom_wait_state = CGC_ROM_ZERO_WAIT_CYCLES;
    }
    /** If the 80 MHz < requested_frequency <= 160 MHz 1 ROM wait state is required) */
    else if (requested_frequency <= CGC_SYS_CLOCK_FREQ_TWO_ROM_WAITS)
    {
        p_args->new_rom_wait_state = CGC_ROM_ONE_WAIT_CYCLES;
    }
    else
    /** Anything above 160 MHz requires 2 ROM wait states */
    {
        p_args->new_rom_wait_state = CGC_ROM_TWO_WAIT_CYCLES;
    }
