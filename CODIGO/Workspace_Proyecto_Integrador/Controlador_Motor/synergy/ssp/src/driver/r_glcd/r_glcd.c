...
static ssp_err_t r_glcd_open_param_check_layer_settings_coordinate(display_cfg_t const * const p_cfg, uint32_t i)
{
    ...
    {
        GLCD_ERROR_RETURN((p_cfg->output.htiming.display_cyc >= (p_cfg->layer[i].coordinate.x)),
                          SSP_ERR_INVALID_LAYER_SETTING);
    }
    else
    {
        GLCD_ERROR_RETURN((p_cfg->output.htiming.display_cyc >= (p_cfg->layer[i].coordinate.x * (-1))),
                          SSP_ERR_INVALID_LAYER_SETTING);
    }
    if (0 <= p_cfg->layer[i].coordinate.y)
    {
        GLCD_ERROR_RETURN((p_cfg->output.vtiming.display_cyc >= (p_cfg->layer[i].coordinate.y)),
                          SSP_ERR_INVALID_LAYER_SETTING);
    }
    else
    {
        GLCD_ERROR_RETURN((p_cfg->output.vtiming.display_cyc >= (p_cfg->layer[i].coordinate.y * (-1))),
                          SSP_ERR_INVALID_LAYER_SETTING);
    }
    return SSP_SUCCESS;
}
                                                   ...
                                                   uint16_t *                        bit_size)
{
    ...
    else if (0 > p_layer->coordinate.x)
    {
        ...
        {   /* If there is no enough cycles for the offset adjustment, simply do not adjust. */
            p_recalculated->hpix_offset = 0;
            p_recalculated->hpix_size   = (uint16_t)(p_input->hsize - (((p_layer->coordinate.x * (-1)) / adj_cycles) * adj_cycles));
        }
        else
        {
            ...
            p_recalculated->hpix_size   =
                    (uint16_t)((p_input->hsize + p_layer->coordinate.x) + ((p_layer->coordinate.x * (-1)) % adj_cycles));
        }
