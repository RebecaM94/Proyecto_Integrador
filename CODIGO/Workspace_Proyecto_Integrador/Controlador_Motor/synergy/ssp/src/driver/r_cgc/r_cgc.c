...
#if (CGC_CFG_PARAM_CHECKING_ENABLE == 1)
        if ((CGC_CLOCK_HOCO == clock_source) || (CGC_CLOCK_MOCO == clock_source)|| (CGC_CLOCK_MAIN_OSC == clock_source) || (CGC_CLOCK_PLL == clock_source))
        {
        	CGC_ERROR_RETURN(CGC_SUBOSC_SPEED_MODE != HW_CGC_OperatingModeGet(gp_system_reg), SSP_ERR_INVALID_MODE);
        }
#endif /* if (CGC_CFG_PARAM_CHECKING_ENABLE == 1) */
    }
    /*  if clock source is LOCO, MOCO */
    if (((clock_source == CGC_CLOCK_LOCO) || (clock_source == CGC_CLOCK_MOCO)))
    {
        HW_CGC_ClockStart(gp_system_reg,clock_source);
    }
    /* if clock source is subclock */
    else if (clock_source == CGC_CLOCK_SUBCLOCK)
    {
        volatile uint32_t timeout;
        timeout = MAX_REGISTER_WAIT_COUNT;
        /* Set SubClockDrive only if clock is not running */
        if (false == HW_CGC_ClockRunStateGet(gp_system_reg, CGC_CLOCK_SUBCLOCK))
        {
            HW_CGC_SubClockDriveSet(gp_system_reg, CGC_CFG_SUBCLOCK_DRIVE);      //Set the SubClock Drive
        }
...
ssp_err_t R_CGC_ClockStop (cgc_clock_t clock_source)
{
    ...
    CGC_ERROR_RETURN(!((CGC_CLOCK_PLL == current_clock) && (HW_CGC_PLLClockSourceGet(gp_system_reg) == clock_source)), SSP_ERR_CLOCK_ACTIVE);
...
ssp_err_t R_CGC_SystemClockSet (cgc_clock_t clock_source, cgc_system_clock_cfg_t const * const p_clock_cfg)
{
    ...
    CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_fclk) && (p_clock_cfg->fclk_div < p_clock_cfg->iclk_div)), SSP_ERR_INVALID_ARGUMENT);

    /* Check if MCU has BCLK, then check if ICLK > BCLK */
    CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_bclk) && (p_clock_cfg->bclk_div < p_clock_cfg->iclk_div)), SSP_ERR_INVALID_ARGUMENT);
    ...
    if(CGC_CLOCK_SUBCLOCK == clock_source)
    {
        ...
        CGC_ERROR_RETURN(!((0U != p_clock_cfg->iclk_div) || ((0U != p_clock_cfg->fclk_div) && (1U == gp_cgc_feature->has_fclk))),
...
ssp_err_t R_CGC_OscStopDetect (void (* p_callback) (cgc_callback_args_t * p_args), bool enable)
{
    if (true == enable)
    {
        ...
        if((0U != gp_cgc_feature->low_speed_max_freq_hz) && (CGC_LOW_SPEED_MODE == operating_mode))
        {
            ...
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_pclka) && (clock_cfg.pclka_div < gp_cgc_feature->low_speed_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has PCLKB and PCLKB divider is greater than minimum low speed clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_pclkb) && (clock_cfg.pclkb_div < gp_cgc_feature->low_speed_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has PCLKC and PCLKC divider is greater than minimum low speed clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_pclkc) && (clock_cfg.pclkc_div < gp_cgc_feature->low_speed_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has PCLKD and PCLKD divider is greater than minimum low speed clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_pclkd) && (clock_cfg.pclkd_div < gp_cgc_feature->low_speed_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has FCLK and FCLK divider is greater than minimum low speed clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_fclk) && (clock_cfg.fclk_div < gp_cgc_feature->low_speed_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has BCLK and BCLK divider is greater than minimum low speed clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_bclk) && (clock_cfg.bclk_div < gp_cgc_feature->low_speed_pclk_div_min)), SSP_ERR_INVALID_MODE);
        ...
        {
        	/* check if the MCU has PCLKA and PCLKA divider is greater than minimum low voltage clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_pclka) && (clock_cfg.pclka_div < gp_cgc_feature->low_voltage_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has PCLKB and PCLKB divider is greater than minimum low voltage clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_pclkb) && (clock_cfg.pclkb_div < gp_cgc_feature->low_voltage_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has PCLKC and PCLKC divider is greater than minimum low voltage clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_pclkc) && (clock_cfg.pclkc_div < gp_cgc_feature->low_voltage_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has PCLKD and PCLKD divider is greater than minimum low voltage clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_pclkd) && (clock_cfg.pclkd_div < gp_cgc_feature->low_voltage_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has FCLK and FCLK divider is greater than minimum low voltage clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_fclk) && (clock_cfg.fclk_div < gp_cgc_feature->low_voltage_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the MCU has BCLK and BCLK divider is greater than minimum low voltage clock divider*/
            CGC_ERROR_RETURN(!((1U == gp_cgc_feature->has_bclk) && (clock_cfg.bclk_div < gp_cgc_feature->low_voltage_pclk_div_min)), SSP_ERR_INVALID_MODE);

            /* check if the ICLK  is greater than minimum low voltage clock divider*/
...
{
    if (1U == gp_cgc_feature->has_sdadc_clock)
    {
