 ...
 * @note        The ssp_signal_t enums and ssp_ip_t are collectively defined for different modules. When used with
 *              the FMI API, the signal and IP should correspond as follows:
 *              1. IP name (ssp_ip_t enum : SSP_IP_<peripheral>)
 *              2. Signal name (ssp_signal_t enum:  SSP_SIGNAL_<peripheral>_ <signal>),
 *                 where <peripheral> is the same as the one used in IP name above.
 **********************************************************************************************************************/
static ssp_err_t R_FMI_EventInfoGet (ssp_feature_t const * const p_feature,
                                     ssp_signal_t                signal,
                                     fmi_event_info_t * const    p_info)
                                                   ...
                                                   fmi_feature_info_t     * const p_info)
{
    ...
    switch (ip_format)
    {
        ...
        case FMI_PRIV_FORMAT_VERBOSE_MULTICHANNEL_COMMON_VARIANT:
            ...
            /* Internal note: Keep uint32_t type to meet coding standard.  p_ip->verbose.variant_data is in bits 0-7
             * and p_ip->verbose.contents is in bits 8-15. Therefore the results fit in the 16-bit bitfield
             * p_info->variant_data.  The upper 16 bits are 0, and they are masked off to prevent a GCC warning. */
            p_info->variant_data = (p_ip->verbose.variant_data | (uint32_t) (p_ip->verbose.contents << 8)) & 0xFFFFU;
