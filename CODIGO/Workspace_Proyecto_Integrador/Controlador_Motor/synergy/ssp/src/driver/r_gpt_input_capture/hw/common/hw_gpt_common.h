 ...
 **********************************************************************************************************************/
__STATIC_INLINE bool HW_GPT_InterruptQuery (GPT_BASE_PTR p_gpt_base)
{
    return (bool) p_gpt_base->GTST_b.TCPFO;
}
...
{
    p_reg->GTIOR_b.OAE = (uint32_t) enable;
}
...
__STATIC_INLINE void HW_GPT_SingleBufferEnable (GPT_BASE_PTR p_gpt_base, gpt_gtioc_t gtio)
{
    if (GPT_GTIOCA == gtio)
    {
        ...
        p_gpt_base->GTBER_b.CCRA = 1;
    }
    else
    {
        /* GTCCRB buffer enable. */
        p_gpt_base->GTBER_b.CCRB = 1;
    }
}
