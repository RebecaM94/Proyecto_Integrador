...
static const int32_t SCI_UART_10_PERCENT_X_1000  = 10000;
                          ...
                          uart_cfg_t const * const p_cfg)
{
    ...
    err = r_sci_uart_baud_calculate(clk_src, p_cfg->baud_rate, &brr, &p_baud_setting, p_mddr);
                           ...
                           uint32_t        const bytes)
{
    ...
    if (NULL != p_ctrl->p_transfer_tx)
    {
        ...
        if (!do_not_reset)
        {
            err = p_ctrl->p_transfer_tx->p_api->reset(p_ctrl->p_transfer_tx->p_ctrl, p_transfer_src, NULL, (uint16_t) num_transfers);
            SCI_UART_ERROR_RETURN(SSP_SUCCESS == err, err);
        }
    }
                             ...
                             uint32_t      const baudrate)
{
    ...
    err = r_sci_uart_baud_calculate(clk_src, baudrate, &brr, &p_baud_setting, p_mddr);
    SCI_UART_ERROR_RETURN(SSP_SUCCESS == err, err);
                                           ...
                                           uint32_t              * p_mddr)
{
    ...
    *pp_baud_setting = &async_baud[0];
    ...
    if (SCI_CLK_SRC_INT == clk_src)
    {
        ...
        int32_t  hit_bit_err = r_sci_uart_brr_mddr_calculate(freq_hz, baudrate, p_brr_value, pp_baud_setting, p_mddr);

        /** Reject any error above 10%. */
        SCI_UART_ERROR_RETURN((hit_bit_err <= SCI_UART_10_PERCENT_X_1000), SSP_ERR_INVALID_ARGUMENT);
    }
